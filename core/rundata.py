...
def poll_results():
    ...
    for data in announced_lga_results_data :
        Data = announced_lga_result.objects.create(result_id=data[0],lga_name=data[1],
    ...
    for data in announced_pu_results_data :
        Data = announced_pu_result.objects.create(result_id =data[0], polling_unit_unique_id=data[1], party_abbreviation=data[2], 
    ...
    for data in lga_data :
        Data = lga.objects.create(uniqueid =data[0], lga_id=data[1], lga_name=data[2],state_id=data[3],lga_description=data[4],
            entered_by_user=data[5], date_entered=data[6], user_ip_address=data[7])
        Data.save()

# party` (`id`, `partyid`, `partyname`)
    for data in party_data :
        Data = party.objects.create(id =data[0], partyid=data[1], partyname=data[2])
        Data.save()
...
# `polling_unit` (`uniqueid`, `polling_unit_id`, `ward_id`, `lga_id`, `uniquewardid`, `polling_unit_number`, `polling_unit_name`, `polling_unit_description`, `lat`, `long`, `entered_by_user`, `date_entered`, `user_ip_address`)
    ...
    for data in polling_unit_data:
        # if data[11] == None:
        #     data[11] = ''
        Data = polling_unit.objects.create(uniqueid = data[0], polling_unit_id = data[1], ward_id = data[2],
